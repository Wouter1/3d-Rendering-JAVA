...
/**
 * An object which represents 3d points or directions, and includes many static
 * methods for 3d math.
 */
public class Vector3 implements Serializable {
	...
	/**
	 * The {@code x} component of the {@code Vector3} in 3d cartesian coordinate
	 * space.
	 */
	public final double x;

	/**
	 * The {@code y} component of the {@code Vector3} in 3d cartensian
	 * coordinate space. Represents the vertical axis.
	 */
	public final double y;

	/**
	 * The {@code z} component of the {@code Vector3} in 3d cartesian coordinate
	 * space. Represents the "depth" axis.
	 */
	public final double z;

	/**
	 * Creates a {@code Vector3} object with the specified {@code x}, {@code y}
	 * and {@code z} components
	 * 
	 * @param xIn the {@code x} component of the {@code Vector3} object being
	 *            made
	 * @param yIn the {@code y} component of the {@code Vector3} object being
	 *            made
	 * @param zIn the {@code z} component of the {@code Vector3} object being
	 *            made
	 */
	...
	/**
	 * Creates a {@code Vector3} object with identical {@code x}, {@code y} and
	 * {@code z} components as the specified vector
	 * 
	...
	/**
	 * A (much) faster alternative to the {@link #getMagnitude} method. This
	 * returns the squared magnitude of the vector, which is recommended for
	 * checking if the magnitude is over a certain value, as the other value can
	 * just be squared, and the resulting comparison will be just as accurate
	 * and much faster than using {@link #getMagnitude}. Example where a is a
	 * {@code double} value and vector is a {@code Vector3} object:
	 * 
	 * <pre>
	...
	/**
	 * adds the specified vector to itself, and also returns itself as the
	 * result after the operation. This does change the {@code x}, {@code y} and
	 * {@code z} components of the vector.
	 * 
	...
	/**
	 * multiplies itself by the specified scalar multiplier. Then returns
	 * itself. There is no divide method so simply multiply by the reciprocal
	 * instead.
	 * 
	...
	/**
	 * Returns a {@code Vector3} object with with value (x*x, y*y, z*z) where x
	 * y and z are the components of the origonal vector.
	 * 
	...
	/**
	 * formats the vector into a string for printing. Uses the format:
	 */
	@Override
...
//============================= static methods ===============================
	...
	/**
	 * the magnitude of vector {@code a} projected into vector {@code b} or vice
	 * versa
	 * 
	 * @param a vector
	 * @param b vector
	...
	/**
	 * the resulting vector is perpendicular to {@code a} and {@code b} note
	 * that the cross product of two unit vectors is not necessarily a unit
	 * vector too
	 * 
	 * @param a vector
	 * @param b vector
...
//	}
	...
	/**
	 * 
	 * @param v another {@link Vector3} to subtract
	 * @return new Vector containing this-v
	 */
	...
	public static Vector3 multiply(Vector3 vector, double scalar) {
		return new Vector3(vector.x * scalar, vector.y * scalar,
				vector.z * scalar);
	}
	...
	public static Vector3 angleToVector(double yaw, double pitch) {
		...
		return new Vector3(Math.sin(yaw) * cosPitch, Math.sin(pitch),
				Math.cos(yaw) * cosPitch);
	}

	/**
	 * Essentially shoots a ray from {@code linePoint} in the direction
	 * {@code lineDirection} and returns the intersection of that ray with the
	 * plane {@code plane}
	 * 
	 ...
	 * @return returns the point at which the vector {@code lineDirection}
	 *         starting at point {@code linePoint} intersects {@code plane}
	 */
	 ...
	 * @return the shortest distance from {@code point} to the line defined by
	 *         the two points, {@code lineP1} and {@code lineP2}
	 */
	...
	/**
	 * returns the shortest distance from {@code point} to {@code plane}.
	 * Basically creates a vector from a point on the plane to the inputted
	 * {@code point} and projects that onto the normal vector of the
	 * {@code plane}, and the magnitude of that is the distance. Removing the
	 * absolute results in the signed distance, which is positive if
	 * {@code point} is on the same side of the plane as it's normal vector, and
	 * negative otherwise.
	 * 
	 * @param point a point
	...
	public static double distanceToPlane(Vector3 point, Plane plane) {
		return Math
				.abs(plane.normal.x * point.x + plane.normal.y * point.y
						+ plane.normal.z * point.z
						- plane.normal.x * plane.pointOnPlane.x
						- plane.normal.y * plane.pointOnPlane.y
						- plane.normal.z * plane.pointOnPlane.z)
	...
	public static Vector3 rotateAroundYaxis(Vector3 point, double angle) {
		...
		return new Vector3(Vector3.dotProduct(new Vector3(cos, 0, sin), point),
				Vector3.dotProduct(new Vector3(0, 1, 0), point),
				Vector3.dotProduct(new Vector3(-sin, 0, cos), point));
	}
	...
	public static Vector3 rotateAroundZaxis(Vector3 point, double angle) {
		double cos = Math.cos(angle);
		double sin = Math.sin(angle);
		return new Vector3(Vector3.dotProduct(new Vector3(cos, -sin, 0), point),
				Vector3.dotProduct(new Vector3(sin, cos, 0), point),
